...
def lineplot(hr_year_csv):
    ...
    #one data series:
    hrdata = pandas.read_csv(hr_year_csv)
    gg = ggplot(hrdata, aes(hrdata["yearID"],hrdata["HR"])) + geom_point(color = 'red') + geom_line(color = 'red') + ggtitle('HR per year') + xlab('Year') + ylab('HR')
    return gg

#multiple categories on same plot:
import pandas
from ggplot import *
...
def lineplot_compare(hr_by_team_year_sf_la_csv):
    ...
    gg = ggplot(data, aes(data['yearID'], data['HR'], color = 'teamID')) + geom_line() + geom_point() + xlab('year') + ylab('HR')
    return gg
